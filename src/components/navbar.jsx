...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 w-full bg-white/80 backdrop-blur">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 sm:px-8 lg:px-12">
          ...
          {items.map((it) => (
            <li key={it.target}>
              <button
                ...
                className="text-sm font-medium text-gray-900 transition-colors hover:text-gray-500"
              >
                {it.label}
              </button>
            </li>
        ...
        <button
          className="lg:hidden rounded-md px-3 py-2 transition-colors hover:bg-gray-100"
          onClick={() => setOpen((v) => !v)}
          ...
          <ul className="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 py-2 space-y-2">
            {items.map((it) => (
              <li key={it.target}>
                <button
                  onClick={() => handleNavClick(it.target)}
                  className="block w-full rounded-md px-3 py-3 text-left text-base font-medium transition-colors hover:bg-gray-100"
                >
                  {it.label}
                </button>
              </li>
